...
Note in the label of the created variable `(0,5]` that the lower limit of the interval is open `(`, which means that only values `greater than 0` are included. You can change this behavior, and do the first range closed using the argument `include.lowest`. While the upper bound is closed`] `, which means that values `less than or equal to 5` are included.

If the vector of values does not meet the criteria described above, the values that do not fit in any interval will be marked as `NA` in the resulting variable. However, this is not the case for the `cut3` function. Due to the `.inf` argument, the supplied cut-off points will be extended by `-Inf` and `Inf`. This way any value is included in the resulting variable.
...
cut3(tbl, var_name = "age", breaks = c(1, 5, 10), .inf = F)
```
...
One of the main novelties of `cut3()` compared to `cut()` is that the vector of `breaks` can be a function that generates the numerical vector of the cuts. The most common use case for this functionality is perhaps if you want to use quantiles to divide the variable. 
...
cut3(tbl, var_name = "age", breaks = 2, .inf = F)
cut3(tbl, var_name = "age", breaks = 2, .inf = F, groups = "sex")
```
